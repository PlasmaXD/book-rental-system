...
const BookRegisterPage = () => {
  ...
  const handleRegisterBook = async (book) => {
    try {
      const response = await axios.post('http://localhost:5000/api/register-book', {
        book,
        user_id: localStorage.getItem('user_id'), // ユーザーIDを使って登録
      });
              ...
              {books.map((book, index) => (
                <ListItem key={index}>
                  <Box p="4" borderWidth="1px" borderRadius="md" boxShadow="md">
                    ...
                    <Button colorScheme="blue" onClick={() => handleRegisterBook(book)}>Register this book</Button>
                  </Box>
                </ListItem>
              ))}
            </List>
